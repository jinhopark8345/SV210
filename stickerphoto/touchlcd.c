...
void LCD_print(unsigned char *fb_mapped) {
  ...
  // PALETTE_START_Y and PALETTE_START_X define the start pos of the palette in
  the touchlcd
  */

  if (!(TOUCHLCD_ON > 0)) { // TOUCHLCD off
    for (coor_y = 0; coor_y < PALETTE_HEIGHT; coor_y++) {
      ystart = (LCD_WIDTH * PALETTE_START_Y + PALETTE_START_X) +
               (LCD_WIDTH * coor_y);
  ...
  /* printf("start point value %d\n", csframe[PALETTE_START_X+1
   * +(PALETTE_START_Y+1) * LCD_WIDTH]); */
}
...
struct lcd_variable init_palette(char *background) {
  ...
  lcdvar.fb_mapped = (unsigned char *)mmap(
      0, lcdvar.mem_size, PROT_READ | PROT_WRITE, MAP_SHARED, lcdvar.fb_fd, 0);
  ...
  for (coor_y = 0; coor_y < PALETTE_HEIGHT; coor_y++) {
    ystart = (screen_width * PALETTE_START_Y + PALETTE_START_X) +
             (screen_width * coor_y);
...
void change_palette_image(unsigned short *rgb) {
  ...
  for (coor_y = 0; coor_y < PALETTE_IMAGE_HEIGHT; coor_y++) {
    ystart = (LCD_WIDTH * PALETTE_IMAGE_START_Y + PALETTE_IMAGE_START_X) +
             (LCD_WIDTH * coor_y);
...
void load_img2LCD(IplImage *load_img) {
  ...
  for (coor_y = 0; coor_y < LCD_HEIGHT; coor_y++) {
    for (coor_x = 0; coor_x < LCD_WIDTH; coor_x++) {
      ...
      g = (load_img
               ->imageData[(coor_y * load_img->widthStep) + coor_x * 3 + 1]);
      r = (load_img
               ->imageData[(coor_y * load_img->widthStep) + coor_x * 3 + 2]);
