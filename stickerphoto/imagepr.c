...
void RGB2cvIMG(IplImage *img, unsigned short *rgb, int ex, int ey) {
  ...
  for (y = 0; y < ey; y++) {
    for (x = 0; x < ex; x++) {
      (img->imageData[(y * img->widthStep) + x * 3]) = (rgb[y * ex + x] & 0x1F)
                                                       << 3; // b
      (img->imageData[(y * img->widthStep) + x * 3 + 1]) =
          ((rgb[y * ex + x] & 0x07E0) >> 5) << 2; // g
      (img->imageData[(y * img->widthStep) + x * 3 + 2]) =
          ((rgb[y * ex + x] & 0xF800) >> 11) << 3; // r
    }
  }
}
...
int detect_face(IplImage *cv_image, unsigned short *cis_rgb,
  ...
  double scale = 1.3;
  CvSize x = cvSize(cvRound(cv_image->width / scale),
                    cvRound(cv_image->height / scale));
  IplImage *gray =
      cvCreateImage(cvSize(cv_image->width, cv_image->height), 8, 1);
  IplImage *small_img = cvCreateImage(x, 8, 1);
  int i;

  static CvScalar colors[] = {{{0, 0, 255}},   {{0, 128, 255}}, {{0, 255, 255}},
                              {{0, 255, 255}}, {{255, 128, 0}}, {{255, 255, 0}},
                              {{255, 0, 0}},   {{255, 0, 255}}};
  ...
  if (cv_image) {
    if (cascade) {
      /* CvSeq *faces = cvHaarDetectObjects(gray, cascade, storage, 1.1, 2,
       * 0,cvSize(30,30)); */
      CvSeq *faces =
          cvHaarDetectObjects(gray, cascade, storage, 1.1, 2,
