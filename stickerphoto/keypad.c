...
void init_keypad() {
  ...
  // /dev/input/event3 파일을 읽기 전용 모드로 열어 // keypad_fd 변수에 파일
  // 기술자를 저장한다.
...
unsigned short read_keypad() {
  ...
  char rtv_input = -1;
  struct input_event
      event_buf[KEYPAD_EVENT_BUF_NUM]; // 몇개의 event 까지 한꺼번에 읽느냐
  struct timeval tv;
  struct timeval newtv;
  /* printf("no problem 1\n"); */
  // event 발생을 KEYPAD_EVENT_BUF_NUM 만큼 읽어들인다.
  read_bytes = read(keypad_fd, event_buf,
                    (sizeof(struct input_event) * KEYPAD_EVENT_BUF_NUM));
  /* printf("no problem 2\n"); */
...
char mapKeypadInput(u_int16_t keypad_input) {
  ...
  case KEYPAD12:
    return SP_BRUSH_RED;
  case KEYPAD13:
    return SP_BRUSH_GREEN;
  case KEYPAD14:
    return SP_BRUSH_BLUE;

  case KEYPAD21:
    return SP_BRUSH_ERASER;
  case KEYPAD22:
    return SP_BRUSH_SIZEUP;
  case KEYPAD23:
    return SP_BRUSH_SIZEDOWN;

  case KEYPAD31:
    return SP_FACEDETECTION;
  case KEYPAD32:
    return SP_GRAYSCALE;
  case KEYPAD33:
    return SP_CAMERA;
