...
typedef struct set_color{
	unsigned int rgbcol; //changed part short --> int
	unsigned short red;
	unsigned short blue;
...
void init_sp_camera(unsigned char *fb_mapped, IplImage* temp_cv_image, char* face_file){
    ...
    printf("c pressed, plain camera starts, save image with 'sw18' \n");
    printf("quit: q \n");
...
void init_sp_gray_scaled(unsigned char *fb_mapped, IplImage* temp_cv_image){
    textlcd_write(user_input, 0,0);
    printf("y pressed\n");
...
void init_stickerphoto(){
  ...
  unsigned short user_input_tmp = 'x';
   ...
   // }
    ...
    if(kbhit()){
      //  user_input = read_keypad(); 
        ...
        case 'y' :
            //Gray Scaling
            init_sp_gray_scaled(fb_mapped, temp_cv_image);
